...
When sandbox mode is enabled, only Phase 1 `called` decisions are converted into sandbox buy orders. `open_failed` and missing-quote decisions are audit-only and never submit broker orders. Shioaji simulation uses common lots only, so `ORDER_CASH_PER_TRADE` must cover at least 1,000 shares at the entry price. Submitted and partially filled orders are reconciled from the ledger on every run, even after their original signal date, and a simulated position is created only after an actual fill. A state-file lock rejects overlapping live runs so the same broker order cannot be submitted concurrently. Sandbox orders and simulated positions are written to the SQLite state DB; the formal tracking JSON and existing website pages remain unchanged.
