...
class OpenEntryCallDecisionTests(unittest.TestCase):
    ...
    def test_render_message_contains_threshold_and_open(self) -> None:
        ...
        self.assertIn("MWP-C 正式追蹤開盤進場提醒", message)
        self.assertIn("2330 台積電", message)
        self.assertIn("訊號日 2026-07-03", message)
        self.assertIn("進場上限 980.0", message)
        self.assertIn("次日開盤 975.0", message)
